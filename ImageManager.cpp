...
void ImageManager::LoadCharacterImageData()
{
	...
	while (!fin.eof())
	{
		...
		characterData.emplace_back(new ImageData2{
			str[0],			//이름저장
			(HBITMAP)LoadImage(NULL, str[1].c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION),	//비트맵저장
			stoi(str[2]),	//Object종류구분
			stoi(str[3]),	//x좌표
...
const pImageData2 ImageManager::GetRedCharacterImageData(SelectData selectData) const
{
	pImageData2 character = NULL;
...
const pImageData2 ImageManager::GetBlueCharacterImageData(SelectData selectData) const
{
	pImageData2 character = NULL;
